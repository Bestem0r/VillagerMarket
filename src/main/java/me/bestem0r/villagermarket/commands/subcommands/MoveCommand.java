...
import me.bestem0r.villagermarket.events.dynamic.MoveToEvent;
...
public class MoveCommand implements SubCommand {
    ...
    public MoveCommand(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void run(CommandSender sender, String[] args) {
        ...
        player.sendMessage(new ColorBuilder(plugin).path("messages.move_villager").addPrefix().build());
        Bukkit.getPluginManager().registerEvents(new MoveEvent(player), plugin);
    }
    ...
    private class MoveEvent implements Listener {
        ...
        public void onInteract(PlayerInteractEntityEvent event) {
            ...
            if (villagerShop != null) {
                player.sendMessage(new ColorBuilder(plugin).path("messages.move_villager_to").addPrefix().build());
                Bukkit.getPluginManager().registerEvents(new MoveToEvent(player, villagerShop), plugin);
            } else {
